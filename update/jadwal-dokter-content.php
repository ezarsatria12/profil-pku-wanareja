...
<div class="table-responsive">
    <table class="table table-bordered align-middle">
        ...
        <tbody>
            <?php foreach ($jadwalDokter as $index => $item): ?>
            <tr>
                <td class="text-center">
                    <?php echo $index + 1 + (($currentPage - 1) * $itemsPerPage); ?>
                </td>
                <td><?php echo htmlspecialchars($item->nama_dokter); ?></td>

                <td class="text-center">
                    <?php if (!empty($item->foto)): ?>
                    <img src="<?php echo BASE_URL . '/uploads/dokter/' . htmlspecialchars($item->foto); ?>"
                        alt="Foto <?php echo htmlspecialchars($item->nama_dokter); ?>"
